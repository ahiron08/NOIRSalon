  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-black">
      <section id="home" className="relative h-screen overflow-hidden">
        <HeroVideo src="/videos/hero.mp4" poster="/images/hero-poster.jpg" play={ready} overlay="rgba(0,0,0,0.55)" />
        <motion.div className="relative z-20 flex h-full items-center" initial="hidden" animate={ready ? 'show' : 'hidden'} variants={{ hidden: {}, show: { transition: { staggerChildren: 0.14, delayChildren: 0.35 } } }}>
          <div className="container-noir w-full">
            <motion.div className="max-w-4xl">
              <motion.p variants={{ hidden: { opacity: 0, y: 22 }, show: { opacity: 1, y: 0, transition: { duration: 1, ease: [0.22, 1, 0.36, 1] } } }} className="eyebrow mb-8 flex items-center gap-4">
              ...
              <h1 className="font-display text-[13.5vw] leading-[0.95] text-white sm:text-7xl lg:text-8xl xl:text-[7rem]">
                {HEADLINE.map((line) => (
                  <span key={line} className="block overflow-hidden">
                    <motion.span className="block" variants={{ hidden: { y: '115%', opacity: 0 }, show: { y: '0%', opacity: 1, transition: { duration: 1.2, ease: [0.22, 1, 0.36, 1] } } }}>{line}</motion.span>
                  </span>
                ))}
              </h1>
              <motion.p variants={{ hidden: { opacity: 0, y: 24 }, show: { opacity: 1, y: 0, transition: { duration: 1, delay: 0.4, ease: [0.22, 1, 0.36, 1] } } }} className="mt-8 max-w-md text-sm font-light leading-relaxed text-noir-muted md:mt-10 md:text-base">
                Experience luxury styling, premium beauty treatments and unmatched elegance crafted by industry-leading professionals.
              </motion.p>
              <motion.div variants={{ hidden: { opacity: 0, y: 24 }, show: { opacity: 1, y: 0, transition: { duration: 1, delay: 0.6, ease: [0.22, 1, 0.36, 1] } } }} className="mt-12 flex flex-wrap items-center gap-4 md:mt-14 md:gap-6">
                <BookButton delay={0} />
        ...
        <div className="absolute right-8 top-1/2 z-20 hidden -translate-y-1/2 lg:right-14 lg:block"><Stats visible={ready} variant="column" /></div>
        <div className="absolute inset-x-0 bottom-28 z-20 hidden sm:block lg:hidden"><div className="container-noir"><Stats visible={ready} variant="row" /></div></div>
      </section>
          ...
          {['Haircut & Styling', 'Signature Facial', 'Bridal Makeup', 'Swedish Massage'].map((name, i) => (
            <motion.div key={name} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1, duration: 0.9, ease: [0.22, 1, 0.36, 1] }} className="group overflow-hidden border border-white/10 bg-white/[0.03]">
              ...
              <div className="p-5">
                ...
                <p className="mt-1 text-xs uppercase tracking-[0.2em] text-noir-gold">From ₹699</p>
              </div>
            </motion.div>
          ))}
        </div>
        <div className="mt-10"><Link to="/home/services" className="inline-flex items-center border border-white/40 px-8 py-3.5 text-xs uppercase tracking-[0.28em] text-white hover:border-noir-gold hover:text-noir-gold">Explore Services</Link></div>
      </Section>

      {/* COMBOS */}
      <Section eyebrow="Curated packages" title="Featured Combos" subtitle="Thoughtfully designed rituals for every occasion.">
        <div className="grid gap-6 md:grid-cols-3">
          {['Bride To Be', 'Weekend Glow', 'Luxury Groom Package'].map((name, i) => (
            <motion.div key={name} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1, duration: 0.9, ease: [0.22, 1, 0.36, 1] }} className="overflow-hidden border border-white/10 bg-white/[0.03]">
              <div className="aspect-[4/5] w-full bg-neutral-900" />
              <div className="p-6">
                <h3 className="font-display text-2xl text-white">{name}</h3>
                <p className="mt-2 text-xs uppercase tracking-[0.2em] text-noir-gold">Save up to ₹5,001</p>
                <Link to="/home/combos" className="mt-5 inline-flex border border-noir-gold/60 px-6 py-2.5 text-[0.7rem] uppercase tracking-[0.25em] text-noir-gold hover:bg-noir-gold hover:text-black">View Details</Link>
              </div>
            </motion.div>
          ))}
        </div>
